...
const { Plugin, TFile, PluginSettingTab, Setting, Menu, Notice } = require("obsidian");
...
module.exports = class StickyPropertiesPlugin extends Plugin {
  async onload() {
  ...
  patchPropertyMenu() {
    ...
    proto.showAtMouseEvent = patched;
    // Restore on unload — but only if no other plugin patched on top of us.
    this.register(() => {
      if (proto.showAtMouseEvent === patched) {
